...
run_CM_calculations = True          # Find CM transitions. If save_transitions is True, the found transitions will be saved in a .csv file

# Location of the Abinit *GSR.nc file. This is the file that contains the k-points,
# eigenvalues, etc
...
# This is the maximum photon energy that will be considered. Note that large values increase computation time.
# The maximum conduction band will be determined by this value and the minimum valence band will be determined with:
# Emin = Emax - TrueBG
Emax = 4

# Tolerance energy difference. This parameter should be studied in a convergence study
etol = 0.01                 # float; default 0.01; This value represents the maximum allowed error
                            # in energy difference (E1f - E1i + Eii - Eff < etol)
                            # Can be set to a typical phonon energy
                            # Note: the error tolerance for momentum conservation is 1E-6 which was tested and found to be a good tolerance

logfilename = 'log.txt'     # String; default 'log.txt'; Defines the name of the log file
save_kfile = False          # Boolean; default False; Save the entire array of kpoints as a csv file or not.
save_CMfile = False         # Boolean; default False; Save the entire array of CM transitions as a csv file or not.
                            # Note that saving the CM file is needed in order to run_transition_density
...
###############################################################################
remove_degenerate = True    # Boolean; default True; Removes degenerate energy bands from calculations. This saves computation time
###############################################################################
# END INPUT - All input is defined above. Below is the code that will be executed
###############################################################################
...
def print_progress(i, imax, start_time, filename):
    ...
    log('i: {}/{}\t ({}%)\nPredicted size: {:1.3f} MB'.format(i, imax, round(progress,1),pred_filesize))
    log('\nRuntime: {} \t Done in: {}'.format(timedelta(seconds=runtime), timedelta(seconds=(runtime)/(progress/100)-runtime)))
    log('-'*75)

def check_moment(i,f,ii,ff,tol=1E-6,debug=False):
    net = abs(kpoints[int(i)]-kpoints[int(f)]+kpoints[int(ii)]-kpoints[int(ff)])[:3]
    if debug:
        log(f"\ti: {kpoints[int(i)]} \n-\tf: {kpoints[int(f)]} \n+\tii: {kpoints[int(ii)]} \n-\tff: {kpoints[(ff)]} = {net}")
...
def check_IBZ(kpoint):
    if 0 <= kpoint[0] <= 0.5*reciprocal_lattice[0,0]:
        if 0.5*reciprocal_lattice[0,1] <= kpoint[1] <= 0.5*reciprocal_lattice[1,1]:
...
def e2red(bandindex):
    ...
    if bandindex < usebands[0] + 1:
        raise ValueError("Bandindex cannot be less than the lowest band used for red_energies")
    if bandindex >= usebands[1]:
        raise ValueError("Bandindex cannot be larger than the highest band used for red_energies")
    if degenerate is True:
        return (bandindex - usebands[0] - 1)//2

def Umklapp(k):
    # I think this should work for all lattices, but I have not tested it for anything other than hexagonal
    # Takes a kpoint (Cartesian coordinates) as input and converts that point to a value within |0.5*reciprocal lattice vector|
    k_new = np.copy(k)
    k_new = k2frac(k_new[:3])
    
    for i,kx in enumerate(k_new):
        if abs(kx) > 0.5:
            k_new[i] = kx +1 * np.sign(kx) * ( np.sign(kx) * -1 * ((kx-0.5)//(1) + 1)) 
...
def find_CM_transitions(krow, energies, BG, Emin, Emax, save_CMfile = True, etol=0.1):
    ...
    for Ei in reversed(range(energies.shape[1])):
        ...
        for Ef in reversed(range(energies.shape[1])):
            ...
            if energies[ki,Ei] > 0 and energies[kf,Ef] <= 0: 
                # Only consider full VB or full CB transitions. If electron goes from CB to VB then there is no CM).
                continue
            for Eii in range(energies.shape[1]):
                if energies[kii,Eii] > 0: # only check energies in VB, since we want to excite a VB electron
                    continue
                # E_min = energies[ki,Ei] - energies[kf, Ef] + energies[kii,Eii] - etol
                # E_max = energies[ki,Ei] - energies[kf, Ef] + energies[kii,Eii] + etol
                for Eff in reversed(range(energies.shape[1])):
                    if energies[kff,Eff] <= 0: # only check energies in CB, since the electron needs to be excited
                        break
                    # if Eff > 0 and (energies[kff,Eff] - energies[kff,Eff-1] < 0.0001): # Do not include if energies are more or less the same
                    #     continue
                    if abs(energies[ki,Ei] - energies[kf, Ef] + energies[kii,Eii] - energies[kff,Eff]) < etol :
                        #dE = energies[ki,Ei] - energies[kf, Ef] + energies[kii,Eii] - energies[kff,Eff]
                        #if abs(dE) < etol:
                        # For CB the initial CM state is an electron
                        if energies[ki,Ei] > 0:
                            new_transition = [[ki,Ei,kf,Ef]]
                        # For a VB transition, the CM carrier is a hole and therefore we change the order of initial & final state
                        if energies[ki,Ei] <= 0:
                            new_transition = [[kf,Ef,ki,Ei]]
                        Ncm[new_transition[0][0],new_transition[0][1]] += 1
                        if save_CMfile is True:
                            transitions = np.append(transitions,new_transition,axis=0)
                        # Need to add a part where it saves truth values for all energies
    if save_CMfile is True:
...
def calculate_CM_transitions(i, kpoints, energies, BG, Emin, Emax, save_kfile=False, kfilename=None, save_CMfile=False, cmfilename=None, chunksize=10000, ktol = 1E-6, etol= 0.01):
    ...
    for f in range(k_range):
        for ii in range(k_range):
            ...
            under = kff[-1] - ktol
            above = kff[-1] + ktol
            a = np.where(np.logical_and(kpoints[:,-1] >= under, kpoints[:,-1] <= above))
            if a[0].shape[0] == 0:
                print(f'{k2frac(kpoints[i,:])} + {k2frac(kpoints[ii,:])} - {k2frac(kpoints[f,:])} = {k2frac(kff)}')
                #print(f'Mismatching k-point: {k2frac(kff)}\n')
            for ff in a[0]:
                # this may be improved with a np.find function. Find index of values between range
                ...
                if not (kpoints[ff,:3] - kff[:3] < 1E-6).all():
                    ...
                    if save_kfile:
                        ktransitions = np.append(ktransitions,ktransition, axis=0)
                    ...
                    if save_CMfile:
                        Ncm_in, cm = find_CM_transitions(ktransition[0], energies, BG, Emin, Emax, save_CMfile = save_CMfile, etol=etol)
                        if not (np.array(cm) == None).any():
                            cmtransitions = np.append(cmtransitions,cm, axis=0)
                    ...
                    if not save_CMfile:
                        Ncm_in = find_CM_transitions(ktransition[0], energies, BG, Emin, Emax, save_CMfile = save_CMfile, etol=etol)
...
def find_photon_energies(transition, energies, Emax=4, nvb=None, ncb=None, max_photons=5, etol=0.0001):
    ...
    if Ei > 0: # CB transitions
        for ix, E in enumerate(reversed(energies[k,:ncb])): # iterate over all VB energies at k
            # if E > 0:
            #     continue
            Ephoton = Ei - E
            if Ephoton > 4:
                break
            CB_photons = np.append(CB_photons,np.array([[Ephoton]]),axis=0) # save photon energy in first col
    
    if Ei <= 0: # VB transitions
        for ix, E in enumerate((energies[k,ncb:])): # iterate over all VB energies at k
            # if E <= 0:
            #     continue
            Ephoton = E - Ei
            if Ephoton > 4:
                break
            VB_photons = np.append(VB_photons,np.array([[Ephoton]]),axis=0) # save photon energy in second col
...
#     global degenerate
#     dfs = [pd.read_csv(VB_photons,names=['E']), pd.read_csv(CB_photons,names=['E'])]
...
#         density[i,:] = kde(E)
#         density[i,:] *= dfs[i].size/ dfs[-1].size # multiply density with filesize to obtain abs number of transitions
#         if degenerate:
#             density[i,:] *= 16       # Multiply with 16 if degenerates were removed

    
#     #density[-1] *= 2 # multiply the total density with 2 since its size is twice as big
...
def find_CMcount(energies,Ncm, BG, Emax=4, dE=0.01, degenerate=True, averaging = False):
    from tqdm import tqdm
    # returns array with 
    ...
    if degenerate:
        factor = 2  # This is needed because Ncm was accounted for degeneracy by multiplying with 16
                    # But if we excite at a given energy, then the electron & hole state has 2 bands it can occupy
                    # The number of transitions that are possible from these degenerate bands is two times smaller
                    # than if we kept viewing it as a single band. In other word, we counted all transitions as if
                    # all degenerate bands were a single band, but if we now consider them two seperate bands again,
                    # naturally we should divide the number of transitions by 2.
    ...
    for k, Ek in enumerate(tqdm(energies)):
        for Eh in range(len(Ek)): # Eh: bandindex of hole
            ...
            for Ee in range(len(Ek)): # Ee: bandindex of electron
                ...
                if Ephoton > Emax:
                    continue
                Ncme, Ncmh = Ncm[k,Ee]/factor, Ncm[k,Eh]/factor       # number of CM first order transitions for electron and hole
    
                
                CMmatrix = np.append(CMmatrix,np.array([[Ephoton,Ncme,Ncmh]]),axis=0)
...
def N2QY(N, rate):
# Convert number of CM states to quantum yield with paramater rate. rate stands for cooling rate to carrier multiplication ratio
# rate = Rcool/Rcm
    return N / (N+rate)
...
def load_and_organize_CMcsv(CMfilename):
    # Loads the CM transitions file as a pandas dataframe, adjusts initial states in valance band (initial state = final state)
    # and sorts dataframe by initial k and E
    CM_transitions = pd.read_csv(CMfilename, names=['ki','Ei','kf','Ef']).sort_values(by=['Ei'])
...
def generate_bxsf_file(filename,abinit_file, band_range=None):
    # modified from abipy iotools
    nband = abinit.nband
    ndivs = abinit.kpoints.mpdivs_shifts[0]
    nkpt = abinit.nkpt
    fermie = abinit.get_e0('fermie').to("Ha")
    energy_array = np.copy(abinit.eigens[0].to("Ha")) - abinit.get_e0('fermie').to("Ha")
...
# Data Initialization
###############################################################################
if __name__ == "__main__":      # This is needed if you want to import functions from this script to another python file
    ...
    log()
    Emin = TrueBG - Emax
    log(f"Emin:\t\t\t\t{Emin} eV\nEmax:\t\t\t\t{Emax} eV\nExperimental Eg:\t{TrueBG} eV\nError tolerance\t\t{etol} eV")
    #log('Limiting energy calculations to {}% of the {} data'.format(datasize,kfilename))
    log(f'remove_denerate\t\t{remove_degenerate}')
    log()
    ...
    try:
        ...
        msg_error('Abinit *GSR.nc file cannot be loaded, check given path for Abinit file')
        sys.exit()
    ...
    log("Gathering k-points in Cartesian coordinates & calculating vector magnitudes..")
    ...
    # The magnitude of the k-vectors are calculated in order to speed up the find_ktransitions function
    ...
    #kpoints = Umklapp(abinit.kpoints.get_cart_coords()) # some kpoints fall outside the BZ, Umklapp corrects them to the BZ
    
    # for i in range(len(kpoints)):
    #     kpoints[i,:3] = np.dot(np.transpose(reciprocal_lattice), abinit.kpoints.frac_coords[i])
    ...
    energies = (np.copy(abinit._eigens[0]) - abinit.get_e0('fermie')).to('eV') # sets VBM as 0 and converts to energies to eV
    # Since the BG is underestimated in DFT, a scissor operator is used to shift
    # the CB so that it matches with experimental data
    ...
    if nmax == 0:
        ...
        Emax = _emax
        msg_warning("Maximum energy is found to be {:1.4f} eV, which is lower than Emax. \n Emax will be adjusted to this value.".format(Emax))
    usebands = (nmin, nmax)
    red_energies = np.copy(energies[:,usebands[0]:usebands[1]])
    log("Limiting energy matrix: lowest band: {}. highest band: {}.".format(usebands[0],usebands[1]))
    #####
    # If remove_degenerate is true, this part will remove degenerate bands from the calculations
    degenerate = False
    if remove_degenerate:
        ...
        if degenerate:
            log("All energies are found to be degenerate. Removing half of the energies for faster computation") # maybe need to add these again at some point for QY calculation
            red_energies = np.delete(red_energies,list(np.arange(0,red_energies.shape[1],2)),1)
    np.savetxt('reduced_energies.csv',red_energies)
    log("Done. Reduced energy array saved in \'reduced_energies.csv\'")
    log("Note: to convert reduced band indices back to real bandindices, use the red2e(bandindex) function.")
    log()
    ...
    if run_CM_calculations is True:
        ...
        if save_kfile:
            if path.exists(kfilename):
                msg_error('{} already exists, please rename the output file before running again'.format(kfilename))
                sys.exit()
                
        if save_kfile:
            if path.exists(CMfilename):
                msg_error('{} already exists, please rename the output file before running again'.format(CMfilename))
                sys.exit()
        ...
        if True: # Run function in parallel mode
            data = Parallel(n_jobs=num_cores)(delayed(calculate_CM_transitions)(i,kpoints,red_energies,TrueBG,
            ...
            Ncm += sum(data)
            if degenerate:
                Ncm *= 16   # Each band is 2fold degenerate. We have 4 energy levels that we compare
        ...
        log("Number of transitions per state data saved in {}".format('Ncm.csv'))
        log('#'*75)
        log()
