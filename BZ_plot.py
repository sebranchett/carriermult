...
path = "C:\\Users\\Sven\\OneDrive\\Studie\\Master\\Jaar 2\\Thesis\\Python scripts\\Data\\MoTe2\\10x10x5_0.90eV_30Ha_Emin"
...
################################################
# The following part only works for hexagonal lattices. Please remove if other lattice is used
...
if top:
    ...
    cbar = fig.colorbar(cm.ScalarMappable(norm=norm,cmap=viridis),ax=ax, shrink=0.5, label='$N_{CM} /; / /; {n_k}^2$')
    cbar.set_label('$N_{CM} \; / \; {n_k}^2$', fontsize=12)
...
if side:
    ...
    for k in range(len(kpoints)):
        if give_color:
            ...
            plot_BZ(bz_lattice,kpoints[k],ax=ax, s= s, color = 'b')
    pl.plot_wigner_seitz(bz_lattice, ax=ax)
    fig.colorbar(cm.ScalarMappable(norm=norm,cmap=viridis),ax=ax, fontsize = 16, shrink=0.5, label='$N_{CM} /; / /; {n_k}^2$')
